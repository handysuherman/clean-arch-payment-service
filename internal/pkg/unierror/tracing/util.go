...
func StartGQLServerTracerSpan(c context.Context, operationName string) (context.Context, opentracing.Span) {
...
func GetKafkaTracingHeadersFromSpanCtx(spanCtx opentracing.SpanContext) []kafka.Header {
	...
	kafkaMsgHeaders := TextMapCarrierToKafkaMessageHeaders(textMapCarrier)
	return kafkaMsgHeaders
}
